...
class PaymongoService
{
    ...
    public function createGcashCheckout(Invoice $invoice): array
    {
        ...
        $reference = 'SLR-' . $invoice->invoice_number . '-' . Str::upper(Str::random(8));
        $origin = rtrim(config('app.url'), '/');
        $payload = ['data' => ['attributes' => [
            'line_items' => [[
                'currency' => 'PHP', 'amount' => (int) round($invoice->balance * 100), 'name' => 'SolarNet invoice ' . $invoice->invoice_number, 'quantity' => 1,
            ]],
            'payment_method_types' => ['gcash'],
            'description' => 'Payment for SolarNet invoice ' . $invoice->invoice_number,
            'reference_number' => $reference,
            'success_url' => $origin . '/customer/billing?payment=success',
            'cancel_url' => $origin . '/customer/billing?payment=cancelled',
            'billing' => ['name' => $invoice->customer->full_name, 'email' => $invoice->customer->email, 'phone' => $invoice->customer->contact_number],
        ]]];
        ...
        PaymongoCheckout::create(['invoice_id' => $invoice->id, 'customer_id' => $invoice->customer_id, 'checkout_session_id' => $data['id'], 'reference_number' => $reference, 'amount' => $invoice->balance]);
        return ['checkout_url' => $checkoutUrl, 'reference_number' => $reference];
    }

    public function markPaidByCheckoutId(string $sessionId): void
    {
        DB::transaction(function () use ($sessionId) {
            $checkout = PaymongoCheckout::with('invoice')->where('checkout_session_id', $sessionId)->lockForUpdate()->first();
            ...
            $payment = $this->invoices->recordPayment($invoice, ['amount' => min($checkout->amount, $invoice->balance), 'payment_method' => 'mobile_money', 'payment_date' => now(), 'transaction_id' => $sessionId, 'reference' => $checkout->reference_number, 'notes' => 'PayMongo GCash checkout']);
