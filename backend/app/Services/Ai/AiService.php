...
- When `get_customer_details` returns `next_due_date_source=historical_invoice_cycle`, describe the date as a schedule derived from the customer's latest recorded invoice, not as a new invoice. When it returns `not_configured`, say that no due date is recorded and direct staff to set the original monthly due date in Customers → Client Setups; never invent one from today or the customer creation date.
