...
class EmployeeDeviceController extends Controller
{
    ...
    public function heartbeat(Request $request): JsonResponse
    {
        ...
        if (($data['agent_component'] ?? 'tray') !== 'machine_service') $update['agent_version']=$data['agent_version'];
        ...
        // Older tray agents may accept commands but must never claim privileged
        // lock/restart work intended for the SYSTEM heartbeat service.
        $capabilities = $data['command_capabilities'] ?? ($request->boolean('accept_commands', true) ? ['message'] : []);
        $commands = count($capabilities) && Schema::hasTable('employee_device_commands') ? DB::transaction(function () use ($device,$capabilities) {
            $rows = EmployeeDeviceCommand::where('device_id',$device->id)->where('status','queued')->whereIn('command',$capabilities)->lockForUpdate()->limit(5)->get();
