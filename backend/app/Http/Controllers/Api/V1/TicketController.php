...
class TicketController extends Controller
{
    ...
    {
        $query = Ticket::with(['customer.servicePlan', 'customer.referralSource.referrer:id,full_name,account_number', 'referral.referrer:id,full_name,account_number', 'referral.cashPayer:id,name', 'router:id,name,location', 'smsAdvisoryCampaign:id,status,recipient_count,sent_count,failed_count,skipped_count', 'assignedTechnician', 'comments', 'histories.user']);
    ...
    {
        $ticket = Ticket::with(['customer.servicePlan', 'customer.referralSource.referrer:id,full_name,account_number', 'referral.referrer:id,full_name,account_number', 'referral.cashPayer:id,name', 'router:id,name,location', 'smsAdvisoryCampaign', 'assignedTechnician', 'comments.user', 'comments.customer', 'histories.user'])->findOrFail($id);
        $ticket->setAttribute('client_notes', $ticket->customer?->notes);
        if ($ticket->ticket_type === 'installation' && $ticket->workflow_status === 'waiting_admin_approval') {
            $ticket->setAttribute('installation_validation', $this->workflow->installationValidation($ticket));
        }
