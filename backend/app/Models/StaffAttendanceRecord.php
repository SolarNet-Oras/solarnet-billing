...
class StaffAttendanceRecord extends Model
{
    ...
    protected $casts = ['work_date'=>'date','clock_in_photo_captured_at'=>'datetime','clock_out_photo_captured_at'=>'datetime','overtime_reviewed_at'=>'datetime','late_minutes'=>'integer','worked_minutes'=>'integer','overtime_minutes'=>'integer','approved_overtime_minutes'=>'integer','clock_in_latitude'=>'float','clock_in_longitude'=>'float','clock_out_latitude'=>'float','clock_out_longitude'=>'float'];
    ...
    /**
     * PostgreSQL timestamp columns do not carry an offset. Attendance punches
     * are deliberately stored as UTC wall-clock values, so they must not be
     * interpreted using the application's Asia/Manila business timezone.
     */
    protected function clockedInAt(): Attribute
    {
        return $this->utcPunchAttribute();
    }

    protected function clockedOutAt(): Attribute
    {
        return $this->utcPunchAttribute();
    }

    private function utcPunchAttribute(): Attribute
    {
        return Attribute::make(
            get: fn ($value) => filled($value) ? Carbon::parse($value, 'UTC')->utc() : null,
