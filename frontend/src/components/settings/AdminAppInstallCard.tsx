...
export function AdminAppInstallCard(): React.JSX.Element {
  ...
  return <section className="overflow-hidden rounded-2xl border border-cyan-500/25 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 p-6 text-slate-100 shadow-xl" data-testid="admin-app-install">
    <div className="flex flex-wrap items-start justify-between gap-5">
      <div className="max-w-2xl">
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-cyan-300"><Download className="h-4 w-4"/>Admin application</div>
        <h2 className="mt-2 text-xl font-bold">Install SolarNet Admin on this device</h2>
        <p className="mt-2 text-sm leading-6 text-slate-300">Installs the secure staff web application on Windows, Android, iPhone, or iPad. It opens the employee login and does not install or expose the customer portal.</p>
    ...
    {help && <div className="mt-5 rounded-xl border border-slate-700 bg-slate-900/80 p-4 text-sm leading-6 text-slate-300">
      {isIos ? <><p className="font-semibold text-white">Install with Safari</p><p className="mt-1"><Share2 className="mr-1 inline h-4 w-4"/>Tap <strong>Share</strong>, choose <strong>Add to Home Screen</strong>, then tap <strong>Add</strong>.</p></> : <><p className="font-semibold text-white">Use your browser install menu</p><p className="mt-1">In Chrome or Edge, open the browser menu and choose <strong>Install SolarNet Admin</strong> or <strong>Install app</strong>. If it is already installed, the browser may hide the install prompt.</p></>}
    </div>}
