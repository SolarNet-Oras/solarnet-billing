...
export function HistoricalCleanupPanel(): React.JSX.Element {
  ...
  const createPreview = async (): Promise<void> => {
    ...
    try { setPreview(await historicalCleanupService.preview({ from_date: fromDate, to_date: toDate, modules, password })); }
  ...
  const execute = async (): Promise<void> => {
    ...
    try {
      await historicalCleanupService.execute(preview.preview_token, confirmation, password);
      setMessage('Cleanup completed. The audit record confirms that zero customer records were deleted.');
      setPreview(null); setConfirmation(''); setModules([]); setPassword('');
    } catch (err: any) { setError(err?.response?.data?.message || 'Cleanup did not run.'); }
    finally { setBusy(false); }
  };

  const close = (): void => { if (busy) return; setOpen(false); setPassword(''); setPreview(null); setConfirmation(''); setError(''); };
  ...
  return <><section className="rounded-xl border border-rose-200 bg-rose-50/40 p-5 dark:border-rose-900/50 dark:bg-rose-950/20">
    ...
    <button type="button" onClick={() => setOpen(true)} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-rose-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-rose-800"><LockKeyhole className="h-4 w-4" /> Open protected cleanup</button>
  </section>
  {open && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/65 p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="cleanup-title"><section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-rose-200 bg-card p-5 shadow-2xl sm:p-6">
    <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><div className="rounded-lg bg-rose-100 p-2 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300"><AlertTriangle className="h-5 w-5" /></div><div><h2 id="cleanup-title" className="font-semibold text-foreground">Historical data cleanup</h2><p className="mt-1 text-sm text-muted-foreground">Password verification is required for preview and deletion.</p></div></div><button type="button" onClick={close} disabled={busy} aria-label="Close cleanup" className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><X className="h-5 w-5" /></button></div>
    ...
    <label className="mt-4 block text-sm font-medium text-foreground">Your Super Administrator password<input type="password" autoComplete="current-password" value={password} onChange={e => { setPassword(e.target.value); setPreview(null); }} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-foreground" /></label>
    <button type="button" onClick={() => void createPreview()} disabled={busy || !fromDate || !toDate || modules.length === 0 || !password} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50"><Eye className="h-4 w-4" />{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Preview eligible records</button>
