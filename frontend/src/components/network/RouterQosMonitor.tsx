...
function SafeQosModeCard({ analysis, selectedMode, clients, selectedCustomerId, onCustomerChange, duration, onDurationChange, pingTarget, onPingTargetChange, preview, testDeployment, action, onPreview, onStartTest, onApply, onRollback }: {
  ...
  const selectedAnalysis = selectedMode === 'safe' ? analysis?.safe : analysis?.full;
  const selectedModeName = selectedMode === 'safe' ? 'Safe QoS' : 'Full QoS';
  const selectedDeployable = selectedMode === 'safe' ? safeDeployable : Boolean(selectedAnalysis?.available);
  const safeTesting = testDeployment?.status === 'safe_testing';
  const safePassed = testDeployment?.status === 'safe_test_passed';
  const selectedBlockReasons = !selectedAnalysis?.available
    ? (selectedAnalysis?.reasons || ['Inspection data is not available yet.'])
    : ['No active SolarNet customer queue currently has the exact queue name, router, and IP target required for a controlled test.'];
  const selectedSuggestions = selectedAnalysis?.suggestions || [];

  return <article className={`mt-5 rounded-xl border p-4 ${selectedDeployable ? 'border-emerald-400/25 bg-emerald-500/5' : 'border-amber-400/40 bg-amber-500/10'}`}>
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${selectedDeployable ? 'text-emerald-300' : 'text-amber-200'}`}>Selected mode: {selectedModeName}</p><h4 className="mt-1 text-base font-semibold text-white">{selectedDeployable ? `${selectedModeName} is ready for the next safety step` : `${selectedModeName} deployment blocked`}</h4><p className="mt-1 max-w-3xl text-xs text-slate-300">Inspection completes before any preview, controlled test, or apply action. No mode selection or inspection ever changes RouterOS.</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${selectedDeployable ? 'bg-emerald-400/15 text-emerald-200' : 'bg-amber-400/15 text-amber-100'}`}>{selectedDeployable ? 'INSPECTION PASSED' : 'DEPLOYMENT BLOCKED'}</span></div>
