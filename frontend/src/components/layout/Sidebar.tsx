...
const navItems: NavItem[] = [
  ...
  { name: 'Leave Requests', path: '/staff-leave', icon: CalendarClock, roles: ['super_admin', 'admin', 'cashier', 'office_admin', 'collector', 'technician', 'noc', 'accounting', 'viewer'] },
        ...
        <div className="space-y-2 border-t border-border p-4">
          ...
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-secondary/70">
              {theme === 'system' ? <Monitor className="h-4 w-4" strokeWidth={1.8} /> : resolvedTheme === 'dark' ? <Moon className="h-4 w-4" strokeWidth={1.8} /> : <Sun className="h-4 w-4" strokeWidth={1.8} />}
            </span>
            <span>{theme === 'system' ? `System Mode (${resolvedTheme})` : theme === 'dark' ? 'Dark Mode' : 'Light Mode'}</span>
          </button>
