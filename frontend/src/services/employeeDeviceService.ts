...
export const employeeDeviceService = {
  ...
  async requestCommand(id:string, input:{command:'message'|'lock'|'restart'|'screen_lock';message?:string;reason:string;password:string;device_confirmation:string;unlock_code?:string}): Promise<void> { await api.post(`/employee-devices/${id}/commands`,input); },
