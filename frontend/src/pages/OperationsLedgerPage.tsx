...
export default function OperationsLedgerPage(): React.JSX.Element {
  ...
  const movementBreakdown = useMemo(() => CASH_DENOMINATIONS.map((denomination) => ({ denomination, count: Math.max(0, Number(movementCashCounts[denomination] || 0)), amount: denomination * Math.max(0, Number(movementCashCounts[denomination] || 0)) })), [movementCashCounts]);
  ...
  const save = async (event: React.FormEvent): Promise<void> => {
    ...
    try {
      await api.post('/financial-entries', { transaction_definition_id: definitionId, amount: Number(amount), entry_date: date, reference: reference || null, idempotency_key: crypto.randomUUID(), cash_breakdown: movementTouchesCash ? movementBreakdown.map(({ denomination, count }) => ({ denomination, count })) : null });
  ...
  const cashMovementFields = movementTouchesCash && <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/30"><p className="text-xs font-bold text-amber-950 dark:text-amber-100">Cash denominations required</p><p className="mt-1 text-xs text-amber-800 dark:text-amber-200">Enter the bills and coins physically added or released. The total must equal the transaction amount.</p><div className="mt-3 grid grid-cols-3 gap-2">{movementBreakdown.map((row) => <label key={row.denomination} className="text-xs font-semibold text-foreground">₱{row.denomination.toLocaleString('en-PH')}<input min="0" max="100000" inputMode="numeric" type="number" value={movementCashCounts[row.denomination] || ''} onChange={(event) => setMovementCashCounts((current) => ({ ...current, [row.denomination]: Math.max(0, Math.trunc(Number(event.target.value) || 0)) }))} className="mt-1 w-full rounded border border-amber-300 bg-white p-2 text-gray-950" /></label>)}</div><p className="mt-3 text-right text-sm font-black text-amber-950 dark:text-amber-100">Denomination total: {formatPHP(movementBreakdown.reduce((total, row) => total + row.amount, 0))}</p></div>;

  const cashReconciliation = !cashPanelOpen ? <button type="button" onClick={() => { const rows = data?.cash_denomination_position?.breakdown ?? data?.cash_count?.breakdown ?? []; setCashCounts(Object.fromEntries(rows.map((row) => [row.denomination, Math.max(0, row.count)]))); setCashPanelOpen(true); }} className="w-full rounded-2xl border border-emerald-300 bg-card p-4 text-left shadow-sm transition hover:border-emerald-500 hover:shadow-md dark:border-emerald-800"><span className="font-bold text-foreground">Cash breakdown</span><span className="ml-3 text-sm text-muted-foreground">{formatPHP(Number(data?.cash_denomination_position?.amount ?? data?.cash_count?.counted_amount ?? 0))} · click to count, edit, and reconcile</span></button> : <div className="fixed inset-0 z-50 overflow-y-auto bg-black/65 p-3 sm:p-6"><div className="mx-auto max-w-5xl rounded-2xl bg-background p-3 shadow-2xl"><div className="mb-3 flex justify-end"><button type="button" onClick={() => setCashPanelOpen(false)} className="rounded-lg border border-input bg-card px-4 py-2 text-sm font-bold text-foreground">Close cash breakdown</button></div><section className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,.8fr)]">
