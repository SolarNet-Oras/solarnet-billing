...
export default function OperationsLedgerPage(): React.JSX.Element {
  ...
  return <DashboardLayout><main className="mx-auto max-w-6xl space-y-6">{cashReconciliation}{detailModal}
    ...
    <section className="rounded-2xl border border-border bg-card p-5"><h2 className="font-semibold text-foreground">New installation incentives</h2><p className="mt-1 text-xs text-muted-foreground">Each approved new client creates a ₱500 incentive pool, divided equally among regular technicians/linemen present that day. The 1st–15th period is released on the 15th; the 16th–30th period is released on the 30th (or the last day of a short month). No wallet is reduced until payroll is released.</p><div className="mt-3 divide-y divide-border">{data?.installation_incentives?.length?data.installation_incentives.map(pool=><article key={pool.id} className="py-3"><div className="flex flex-wrap justify-between gap-2"><div><b className="text-foreground">{pool.ticket?.customer?.full_name||'New client installation'}</b><p className="text-xs text-muted-foreground">{pool.ticket?.ticket_number} · {new Date(pool.work_date).toLocaleDateString('en-PH')} · {pool.eligible_count} eligible present staff</p></div><b className="text-emerald-700 dark:text-emerald-300">{formatPHP(pool.pool_amount)}</b></div><div className="mt-2 flex flex-wrap gap-2">{pool.allocations.length?pool.allocations.map(allocation=><span key={allocation.id} className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{allocation.user?.name}: {formatPHP(allocation.share_amount)}</span>):<span className="text-xs font-semibold text-amber-700 dark:text-amber-300">No regular technician/lineman was present; no share allocated.</span>}</div></article>):<p className="py-4 text-sm text-muted-foreground">No approved new-client installation incentives for this period.</p>}</div></section>
