...
export default function OperationsLedgerPage(): React.JSX.Element {
  ...
  const list = (items: Entry[], empty: string) => <div className="mt-3 divide-y divide-border text-sm">{items.length ? items.map((item) => {
    ...
    return <div key={item.id} className="flex items-start justify-between gap-3 py-2"><span><b>{item.category || item.customer?.full_name || 'Client collection'}</b><span className="ml-2">{item.description || item.customer?.account_number || ''}</span><span className="ml-2 text-muted-foreground">{METHOD_LABELS[item.payment_method] ?? item.payment_method}</span><small className="mt-1 block text-xs text-muted-foreground">{actor}</small></span><span className="flex shrink-0 items-center gap-2"><b>{formatPHP(item.amount)}</b><button type="button" onClick={() => setDetailEntry(item)} className="inline-flex items-center gap-1 rounded-lg border border-input bg-background px-2 py-1 text-xs font-semibold text-foreground hover:bg-muted" aria-label={`View details for ${item.category || item.customer?.full_name || 'transaction'}`}><Eye className="h-3.5 w-3.5" />View details</button></span></div>;
