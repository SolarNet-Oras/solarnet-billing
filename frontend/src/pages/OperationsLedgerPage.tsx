...
export default function OperationsLedgerPage(): React.JSX.Element {
  ...
  const [topUp, setTopUp] = useState({ amount: '', reference: '', notes: '', confirmation: '' });
  const [isSavingTopUp, setIsSavingTopUp] = useState(false);

  const activeDefinitions = useMemo(() => definitions.filter((definition) => definition.active !== false && (canTopUpCash || !(definition.effect_type === 'cash_in' && !definition.source_wallet && definition.destination_wallet === 'cash'))), [definitions, canTopUpCash]);
  ...
  const saveCashTopUp = async (event: React.FormEvent): Promise<void> => {
    event.preventDefault();
    if (isSavingTopUp || topUp.confirmation !== 'ADD CASH TOP UP') return;
    ...
    try {
      ...
      setTopUp({ amount: '', reference: '', notes: '', confirmation: '' });
