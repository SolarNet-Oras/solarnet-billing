...
type Entry = {
  id: string; description?: string; category?: string; type?: string; amount: number; payment_method: string;
  entry_date?: string; payment_date?: string; created_at?: string; updated_at?: string;
  reference?: string | null; transaction_id?: string | null; notes?: string | null;
  effect_type?: string | null; source_wallet?: string | null; destination_wallet?: string | null;
  payment_number?: string; cash_counted_amount?: number | null; cash_change_amount?: number | null;
  cash_breakdown?: Array<{ denomination: number; count: number; amount?: number; kind?: 'bill' | 'coin' }> | null;
  customer?: { full_name: string; account_number: string };
  collector?: { name: string };
  receiver?: { name: string };
  recorder?: { name: string };
  remittance?: { id?: string; status?: string; submitted_at?: string | null; liquidated_at?: string | null; received_at?: string | null; liquidator?: { name: string } };
};
...
export default function OperationsLedgerPage(): React.JSX.Element {
  ...
  const [masterForm, setMasterForm] = useState({ type: '', description: '', effect_type: 'expense', payment_method: 'cash', source_wallet: '' });
  ...
  const load = async (): Promise<void> => {
    ...
    if (ledger.status === 'rejected' && master.status === 'rejected') {
      ...
      setError('Transaction choices loaded, but the Daily Operations ledger could not be loaded.');
    } else if (master.status === 'rejected') {
      setError('Daily Operations loaded, but the transaction dropdowns could not be loaded. Check this user\'s View Payments permission.');
    } else {
      setError('');
    }
  };
  useEffect(() => { void load(); }, [date, month, periodMode, masterOpen, canManageDropdowns]);
  ...
  const saveMaster = async (event: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setMasterForm({ type: '', description: '', effect_type: 'expense', payment_method: 'cash', source_wallet: '' });
      await load();
  ...
  const list = (items: Entry[], empty: string) => <div className="mt-3 divide-y divide-border text-sm">{items.length ? items.map((item) => {
    ...
    return <div key={item.id} className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-2 py-2"><span className="min-w-0 break-words"><b>{item.category || item.customer?.full_name || 'Client collection'}</b><span className="ml-2 break-words">{item.description || item.customer?.account_number || ''}</span><span className="ml-2 break-words text-muted-foreground">{METHOD_LABELS[item.payment_method] ?? item.payment_method}</span><small className="mt-1 block break-words text-xs text-muted-foreground">{actor}</small></span><span className="flex max-w-[7rem] flex-col items-end gap-1"><b className="max-w-full break-all text-right text-xs sm:text-sm">{formatPHP(item.amount)}</b><button type="button" onClick={() => setDetailEntry(item)} className="inline-flex h-7 max-w-full items-center gap-1 rounded-md border border-input bg-background px-1.5 text-[10px] font-semibold leading-none text-foreground hover:bg-muted sm:px-2 sm:text-xs" aria-label={`View details for ${item.category || item.customer?.full_name || 'transaction'}`} title="View transaction details"><Eye className="h-3 w-3 shrink-0" /><span className="hidden sm:inline">Details</span></button></span></div>;
  ...
  const detailModal = detailEntry && <div className="fixed inset-0 z-[60] overflow-y-auto bg-black/65 p-3 sm:p-6"><article className="mx-auto max-w-2xl rounded-2xl border border-border bg-card shadow-2xl"><header className="flex items-start justify-between gap-4 border-b border-border p-5"><div><p className="text-xs font-bold uppercase tracking-wide text-primary">Transaction details</p><h2 className="mt-1 text-xl font-black text-foreground">{detailEntry.category || detailEntry.customer?.full_name || detailEntry.description || 'Daily operation'}</h2><p className="mt-1 text-sm text-muted-foreground">Record ID: {detailEntry.id}</p></div><button type="button" onClick={() => setDetailEntry(null)} className="rounded-lg border border-input p-2 text-foreground hover:bg-muted" aria-label="Close transaction details"><X className="h-5 w-5" /></button></header><div className="space-y-5 p-5"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-muted p-3"><p className="text-xs text-muted-foreground">Amount</p><p className="text-xl font-black text-foreground">{formatPHP(Number(detailEntry.amount))}</p></div><div className="rounded-xl bg-muted p-3"><p className="text-xs text-muted-foreground">Payment channel</p><p className="font-bold text-foreground">{METHOD_LABELS[detailEntry.payment_method] ?? detailEntry.payment_method ?? 'Not recorded'}</p></div></div><dl className="grid gap-x-5 gap-y-3 text-sm sm:grid-cols-2">{[
    ['Type', detailEntry.type || detailEntry.effect_type], ['Description', detailEntry.description], ['Customer', detailEntry.customer ? `${detailEntry.customer.full_name} (${detailEntry.customer.account_number})` : null], ['Payment receipt', detailEntry.payment_number], ['Reference', detailEntry.reference], ['Transaction ID', detailEntry.transaction_id], ['Source wallet', detailEntry.source_wallet], ['Destination wallet', detailEntry.destination_wallet], ['Recorded by', detailEntry.recorder?.name || detailEntry.receiver?.name || detailEntry.collector?.name], ['Entry/payment date', detailEntry.entry_date || detailEntry.payment_date], ['Created timestamp', detailEntry.created_at], ['Updated timestamp', detailEntry.updated_at], ['Liquidated timestamp', detailEntry.remittance?.liquidated_at], ['Received timestamp', detailEntry.remittance?.received_at], ['Notes', detailEntry.notes],
  ].filter((row) => row[1]).map(([label, value]) => <div key={label}><dt className="text-xs font-semibold text-muted-foreground">{label}</dt><dd className="mt-0.5 break-words font-medium text-foreground">{String(value).includes('T') && (label.includes('timestamp') || label.includes('date')) ? new Date(String(value)).toLocaleString('en-PH') : String(value)}</dd></div>)}</dl>{detailEntry.cash_breakdown && detailEntry.cash_breakdown.some((line) => Number(line.count) > 0) && <section><h3 className="font-bold text-foreground">Cash denominations</h3><div className="mt-2 overflow-hidden rounded-xl border border-border"><table className="w-full text-sm"><thead className="bg-muted text-left text-xs uppercase text-muted-foreground"><tr><th className="p-3">Pieces</th><th className="p-3">Denomination</th><th className="p-3">Type</th><th className="p-3 text-right">Amount</th></tr></thead><tbody>{detailEntry.cash_breakdown.filter((line) => Number(line.count) > 0).map((line, index) => <tr key={`${line.denomination}-${line.kind || 'cash'}-${index}`} className="border-t border-border"><td className="p-3 font-bold">{line.count}</td><td className="p-3">{formatPHP(line.denomination)}</td><td className="p-3 capitalize">{line.kind || (line.denomination >= 20 ? 'bill' : 'coin')}</td><td className="p-3 text-right font-bold">{formatPHP(Number(line.amount ?? line.denomination * line.count))}</td></tr>)}</tbody></table></div><p className="mt-2 text-right text-sm font-black text-foreground">Total: {formatPHP(detailEntry.cash_breakdown.reduce((sum, line) => sum + Number(line.amount ?? line.denomination * line.count), 0))}</p></section>}{detailEntry.cash_counted_amount != null && <p className="rounded-lg bg-muted p-3 text-sm text-foreground">Cash counted: <b>{formatPHP(Number(detailEntry.cash_counted_amount))}</b>{Number(detailEntry.cash_change_amount || 0) > 0 ? <> · Change: <b>{formatPHP(Number(detailEntry.cash_change_amount))}</b></> : null}</p>}</div></article></div>;
  ...
  return <DashboardLayout><main className="mx-auto max-w-6xl space-y-6">{cashReconciliation}{detailModal}
    <div className="flex flex-wrap justify-between gap-3"><div><h1 className="text-3xl font-bold text-foreground">Daily Operations</h1><p className="mt-1 text-muted-foreground">{periodMode === 'month' ? `Viewing all transactions for ${month}.` : `Viewing transactions for ${date}.`} New records use the selected date.</p></div><div className="flex flex-wrap gap-2">{canTopUpCash && <button type="button" onClick={() => setShowCashTopUp((open) => !open)} className="self-end rounded-lg bg-amber-500 px-3 py-2 text-sm font-bold text-slate-950 shadow-sm hover:bg-amber-400">{showCashTopUp ? 'Cancel wallet top-up' : 'Top up Cash / GCash / Banks'}</button>}{canManageDropdowns && <button type="button" onClick={() => setMasterOpen((open) => !open)} className="rounded-lg border border-input bg-background px-3 py-2 text-sm font-medium text-foreground">{masterOpen ? 'Close dropdown settings' : 'Manage dropdowns'}</button>}<label className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white shadow-sm">Specific date<input type="date" value={date} onChange={(event) => { setDate(event.target.value); setPeriodMode('day'); }} className="mt-1 block w-full cursor-pointer rounded-md border border-blue-300 bg-white px-2 py-1.5 text-sm font-semibold text-blue-950 [color-scheme:light]" /></label><label className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white shadow-sm">Month<input type="month" value={month} onChange={(event) => { setMonth(event.target.value); setPeriodMode('month'); }} className="mt-1 block w-full cursor-pointer rounded-md border border-emerald-300 bg-white px-2 py-1.5 text-sm font-semibold text-emerald-950 [color-scheme:light]" /></label><button type="button" onClick={() => { setDate(new Date().toISOString().slice(0, 10)); setPeriodMode('day'); }} className="self-end rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Today</button>{cashBreakdownButton}</div></div>
    {error && <p className="rounded-lg bg-red-50 p-3 text-red-700">{error}</p>}
    <section className="rounded-2xl border border-border bg-card p-5"><h2 className="font-semibold text-foreground">New installation incentives</h2><p className="mt-1 text-xs text-muted-foreground">Each approved new client creates a ₱500 incentive pool, divided equally among regular technicians/linemen who are still clocked in when the installation is approved. Staff who already clocked out receive no share. The 1st–15th period is released on the 15th; the 16th–30th period is released on the 30th (or the last day of a short month). No wallet is reduced until payroll is released.</p><div className="mt-3 divide-y divide-border">{data?.installation_incentives?.length?data.installation_incentives.map(pool=><article key={pool.id} className="py-3"><div className="flex flex-wrap justify-between gap-2"><div><b className="text-foreground">{pool.ticket?.customer?.full_name||'New client installation'}</b><p className="text-xs text-muted-foreground">{pool.ticket?.ticket_number} · {new Date(pool.work_date).toLocaleDateString('en-PH')} · {pool.eligible_count} eligible on-duty staff</p></div><b className="text-emerald-700 dark:text-emerald-300">{formatPHP(pool.pool_amount)}</b></div><div className="mt-2 flex flex-wrap gap-2">{pool.allocations.length?pool.allocations.map(allocation=><span key={allocation.id} className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{allocation.user?.name}: {formatPHP(allocation.share_amount)}</span>):<span className="text-xs font-semibold text-amber-700 dark:text-amber-300">No regular technician/lineman was clocked in; no share allocated.</span>}</div></article>):<p className="py-4 text-sm text-muted-foreground">No approved new-client installation incentives for this period.</p>}</div></section>
    ...
    {canManageDropdowns && masterOpen && <section className="rounded-2xl border border-border bg-card p-5">
      <h2 className="font-semibold text-foreground">Transaction dropdown settings</h2>
      <p className="mt-1 text-sm text-muted-foreground">Define the record nature separately from its wallet. Cash, GCash, BPI, and Landbank are channels—not expense names.</p>
      <div className="mt-5 grid gap-6 lg:grid-cols-2">
        <form onSubmit={saveMaster} className="grid gap-3 sm:grid-cols-2">
          <label className="text-xs font-semibold text-muted-foreground">Record nature<select required value={masterForm.effect_type} onChange={(event) => { const effect_type = event.target.value; setMasterForm({ ...masterForm, effect_type, payment_method: effect_type === 'expense' ? 'cash' : 'add_to_cash', source_wallet: '' }); }} className="mt-1 w-full rounded-lg border border-input bg-background p-2 text-foreground"><option value="expense">Expense / money out</option><option value="cash_in">New funds / money in</option><option value="transfer">Internal wallet transfer</option></select></label>
          <label className="text-xs font-semibold text-muted-foreground">Type<input required value={masterForm.type} onChange={(event) => setMasterForm({ ...masterForm, type: event.target.value })} placeholder="e.g. Owner funding" className="mt-1 w-full rounded-lg border border-input bg-background p-2 text-foreground" /></label>
          <label className="text-xs font-semibold text-muted-foreground">Description<input required value={masterForm.description} onChange={(event) => setMasterForm({ ...masterForm, description: event.target.value })} placeholder="e.g. Working capital" className="mt-1 w-full rounded-lg border border-input bg-background p-2 text-foreground" /></label>
          <label className="text-xs font-semibold text-muted-foreground">{masterForm.effect_type === 'expense' ? 'Paid from wallet' : 'Destination wallet'}<select value={masterForm.payment_method} onChange={(event) => setMasterForm({ ...masterForm, payment_method: event.target.value })} className="mt-1 w-full rounded-lg border border-input bg-background p-2 text-foreground">{masterForm.effect_type === 'expense' ? <><option value="cash">Cash</option><option value="gcash">GCash</option><option value="bank_bpi">BPI</option><option value="bank_landbank">Landbank</option></> : <><option value="add_to_cash">Cash</option><option value="add_to_gcash">GCash</option><option value="deposit_to_bpi">BPI</option><option value="deposit_to_landbank">Landbank</option></>}</select></label>
          {masterForm.effect_type === 'transfer' && <label className="text-xs font-semibold text-muted-foreground sm:col-span-2">Source wallet<select required value={masterForm.source_wallet} onChange={(event) => setMasterForm({ ...masterForm, source_wallet: event.target.value })} className="mt-1 w-full rounded-lg border border-input bg-background p-2 text-foreground"><option value="">Select source wallet</option><option value="cash">Cash</option><option value="gcash">GCash</option><option value="bpi">BPI</option><option value="landbank">Landbank</option></select></label>}
          <button disabled={isSavingMaster} className="rounded-lg bg-primary p-2 font-semibold text-primary-foreground disabled:opacity-50 sm:col-span-2">{isSavingMaster ? 'Saving…' : 'Add dropdown option'}</button>
        </form>
        <div className="max-h-72 overflow-auto rounded-lg border border-border"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-card text-muted-foreground"><tr><th className="p-2">Type</th><th className="p-2">Description</th><th className="p-2">Nature</th><th className="p-2">Wallet</th><th className="p-2">Status</th><th className="p-2"></th></tr></thead><tbody>{definitions.map((definition) => <tr key={definition.id} className="border-t border-border"><td className="p-2">{definition.type}</td><td className="p-2">{definition.description}</td><td className="p-2">{definition.effect_type === 'cash_in' ? 'Money in' : definition.effect_type === 'transfer' ? 'Transfer' : 'Expense'}</td><td className="p-2">{METHOD_LABELS[definition.payment_method] ?? definition.payment_method}</td><td className="p-2">{definition.active === false ? 'Inactive' : 'Active'}</td><td className="p-2">{definition.active !== false && <button type="button" onClick={() => void deactivateMaster(definition)} className="text-red-600 hover:underline">Remove</button>}</td></tr>)}</tbody></table></div>
      </div>
    </section>}
