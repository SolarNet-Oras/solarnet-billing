...
type Detail = {
  ...
  payments: Array<{ id: string; amount: number; payment_method: string; payment_date: string; reference?: string; transaction_id?: string; invoice?: { invoice_number: string }; paymongoCheckout?: { payment_intent_id?: string; paymongo_payment_id?: string; status?: string; reference_number?: string } | null }>;
  ...
  final_grace_warnings: Array<{ id: string; invoice?: { invoice_number: string } | null; channel: 'sms' | 'email'; amount: number; original_due_date: string; grace_period_start: string; grace_period_end: string; suspension_at: string; provider_message_id?: string | null; status: string; sent_at?: string | null; failure_reason?: string | null; created_at: string }>;
};
...
export default function CustomerDetailPage(): React.JSX.Element {
  ...
  return <DashboardLayout><main className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-2xl border border-border bg-card p-5"><h2 className="flex items-center gap-2 font-semibold text-foreground"><Wallet className="h-4 w-4 text-primary" /> Transaction log</h2><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs uppercase text-muted-foreground"><tr><th className="pb-2">Date</th><th className="pb-2">Invoice</th><th className="pb-2">Method</th><th className="pb-2">PayMongo status / ID</th><th className="pb-2">Amount</th></tr></thead><tbody>{detail.payments.length ? detail.payments.map((payment) => <tr key={payment.id} className="border-t border-border"><td className="py-2">{payment.payment_date}</td><td>{payment.invoice?.invoice_number || 'Not recorded'}</td><td className="capitalize">{payment.paymongoCheckout ? 'QR Ph' : payment.payment_method}</td><td>{payment.paymongoCheckout ? <><span className="capitalize">{payment.paymongoCheckout.status || 'pending'}</span><small className="block text-xs text-muted-foreground">{payment.paymongoCheckout.paymongo_payment_id || payment.paymongoCheckout.payment_intent_id || payment.paymongoCheckout.reference_number || 'Not recorded'}</small></> : payment.reference || payment.transaction_id || 'Not recorded'}</td><td>{formatPHP(payment.amount)}</td></tr>) : <tr><td colSpan={5} className="py-5 text-muted-foreground">No payments recorded yet.</td></tr>}</tbody></table></div></section>
    ...
    <section className="rounded-2xl border border-border bg-card p-5"><h2 className="flex items-center gap-2 font-semibold text-foreground"><BellRing className="h-4 w-4 text-primary" /> Final grace-period warning audit</h2><p className="mt-1 text-sm text-muted-foreground">One SMS and one email are allowed for the suspension-triggering invoice. The matching Web Push status appears in the notification audit above.</p><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs uppercase text-muted-foreground"><tr><th className="pb-2">Invoice</th><th className="pb-2">Channel</th><th className="pb-2">Balance</th><th className="pb-2">Due / grace end</th><th className="pb-2">Suspension from</th><th className="pb-2">Status</th><th className="pb-2">Delivery audit</th></tr></thead><tbody>{detail.final_grace_warnings.length ? detail.final_grace_warnings.map((warning) => <tr key={warning.id} className="border-t border-border"><td className="py-2">{warning.invoice?.invoice_number || 'Archived invoice'}</td><td className="uppercase">{warning.channel}</td><td>{formatPHP(warning.amount)}</td><td>{warning.original_due_date} / {warning.grace_period_end}</td><td>{new Date(warning.suspension_at).toLocaleString('en-PH')}</td><td className="capitalize">{warning.status}</td><td>{warning.provider_message_id ? `Provider ID: ${warning.provider_message_id}` : warning.failure_reason || (warning.sent_at ? `Sent ${new Date(warning.sent_at).toLocaleString('en-PH')}` : 'Awaiting delivery')}</td></tr>) : <tr><td colSpan={7} className="py-5 text-muted-foreground">No final grace-period warnings have been scheduled for this customer.</td></tr>}</tbody></table></div></section>
