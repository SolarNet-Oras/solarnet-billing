...
const MetricTile = ({ label, value, Icon, tone }: { label: string; value: string | number; Icon: LucideIcon; tone: string }) => (
  <div className="group relative overflow-hidden rounded-xl border border-border/70 bg-card p-3 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5">
    <div className={`absolute -right-5 -top-5 h-16 w-16 rounded-full opacity-20 blur-2xl ${tone}`} />
    <div className="relative flex items-start justify-between gap-3">
      <div>
        <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{label}</p>
        <p className="mt-1 text-xl font-bold tracking-tight text-foreground tabular-nums">{value}</p>
      </div>
      <div className={`flex h-8 w-8 items-center justify-center rounded-lg text-white shadow-md ${tone}`}>
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto flex max-w-7xl flex-col gap-6 pb-10">
        <section className="relative overflow-hidden rounded-2xl border border-primary/15 bg-gradient-to-br from-slate-950 via-slate-900 to-primary/90 px-5 py-4 text-white shadow-xl shadow-primary/15 md:px-6">
          ...
          <div className="relative flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-cyan-200">
              ...
              <h1 className="text-xl font-semibold tracking-tight md:text-2xl">Client & billing overview</h1>
              <p className="mt-1 max-w-2xl text-xs text-slate-300">A real-time view of your subscribers, collections, and network readiness.</p>
            </div>
            <button
              type="button"
              onClick={() => fetchMetrics(true)}
              disabled={refreshing}
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-xs font-semibold text-white backdrop-blur transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-70"
            >
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
            ...
            <div className="mt-3 divide-y divide-border/70">
              ...
              ].map(([label, value]) => <div key={label} className="flex items-center justify-between py-2 text-sm"><span className="text-muted-foreground">{label}</span><span className="font-semibold tabular-nums text-foreground">{value}</span></div>)}
            </div>
          </div>
        </section>
