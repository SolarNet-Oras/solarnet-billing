...
type PreviewRow = {
  ...
  historical_installation_date_valid: boolean;
  raw_installation_date: string | number | null;
  action: 'REGISTER' | 'UPDATE' | 'REQUIRES REVIEW';
  exclusion_reasons: string[];
  existing_customer: { id: string; account_number: string; installation_date: string | null } | null;
};
...
const ClientMigrationPage: React.FC = () => {
  ...
  const updateExisting = async (item: PreviewRow): Promise<void> => {
    if (!item.existing_customer || item.match_status !== 'EXACT MAC MATCH') return;
    setRegisteringRow(item.row);
    setError('');
    try {
      await api.patch(`/super-admin/client-migrations/customers/${item.existing_customer.id}`, {
        full_name: item.record.Name,
        address: item.record.Address,
        installation_date: item.record['Installation Date'],
        service_plan_id: item.service_plan?.id,
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {preview.length > 0 && <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          <div className="border-b border-border p-5">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Audit {auditId}. Exact MAC matches can register new clients or safely update imported details on an existing matched client.</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-muted/60 text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3">Row</th><th className="px-4 py-3">Client</th><th className="px-4 py-3">Excel / current / new install date</th><th className="px-4 py-3">Imported balance</th><th className="px-4 py-3">MAC</th><th className="px-4 py-3">Service plan</th><th className="px-4 py-3">Match result</th><th className="px-4 py-3">Eligibility</th><th className="px-4 py-3">Action</th></tr></thead>
              <tbody className="divide-y divide-border">
                {preview.map((item) => <tr key={item.row}><td className="px-4 py-3">{item.row}</td><td className="px-4 py-3 font-medium text-foreground">{item.record.Name || 'Lease registration'}<br /><span className="text-xs font-normal text-muted-foreground">{item.record.Address || 'To be updated'}</span></td><td className="px-4 py-3 text-xs">Excel: <strong>{item.record['Installation Date'] || 'Missing'}</strong>{!item.record['Installation Date'] && item.raw_installation_date !== null && <><br />Source: {String(item.raw_installation_date)}</>}<br />Current: {item.existing_customer?.installation_date || '—'}<br />New: {item.record['Installation Date'] || 'Requires review'}</td><td className="px-4 py-3 text-xs">Previous: ₱{Number(item.record['Previous balance'] || 0).toFixed(2)}<br />Current: ₱{Number(item.record['Current balance'] || 0).toFixed(2)}</td><td className="px-4 py-3 font-mono text-xs">{item.record.Leases}</td><td className="px-4 py-3">{item.service_plan ? `${item.service_plan.name} · ₱${item.service_plan.price}` : <span className="text-muted-foreground">Lease rate preserved; assign plan later</span>}</td><td className="px-4 py-3">{item.match_status}</td><td className="px-4 py-3">{item.registration_eligible ? <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200">Ready to register</span> : item.update_eligible ? <span className="rounded-full bg-sky-100 px-2 py-1 text-xs font-medium text-sky-800 dark:bg-sky-900/40 dark:text-sky-200">Existing client</span> : <span className="text-xs text-amber-700">{item.exclusion_reasons.join(' ')}</span>}</td><td className="px-4 py-3">{registeredRows.includes(item.row) ? <span className="inline-flex items-center gap-1 text-emerald-700"><Check className="h-4 w-4" /> {item.action === 'UPDATE' ? 'Updated' : 'Registered'}</span> : item.registration_eligible ? <button disabled={registeringRow === item.row} onClick={() => void registerMatch(item)} className="rounded-md bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">{registeringRow === item.row ? 'Registering…' : 'Register client'}</button> : item.update_eligible ? <button disabled={registeringRow === item.row} onClick={() => void updateExisting(item)} className="rounded-md bg-sky-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-sky-700 disabled:opacity-50">{registeringRow === item.row ? 'Updating…' : 'Update imported details'}</button> : <span className="text-xs text-muted-foreground">Requires review</span>}</td></tr>)}
              </tbody>
            </table>
          </div>
        </section>}
