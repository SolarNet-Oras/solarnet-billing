          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-border">
                {preview.map((item) => <tr key={item.row}><td className="px-4 py-3">{item.row}</td><td className="px-4 py-3 font-medium text-foreground">{item.record.Name}</td><td className="px-4 py-3 text-xs">{item.record.Address}<br />Installed: {item.record['Installation Date'] || 'Missing'}</td><td className="px-4 py-3 text-xs">Previous: ₱{Number(item.record['Previous balance'] || 0).toFixed(2)}<br />Current: ₱{Number(item.record['Current balance'] || 0).toFixed(2)}</td><td className="px-4 py-3 font-mono text-xs">{item.record.Leases}</td><td className="px-4 py-3">{item.service_plan ? `${item.service_plan.name} · ₱${item.service_plan.price}` : <span className="text-amber-700">No active plan found</span>}</td><td className="px-4 py-3">{item.match_status}</td><td className="px-4 py-3">{item.requires_confirmation ? <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-medium text-amber-800 dark:bg-amber-900/40 dark:text-amber-200">Confirmation required</span> : <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200">Ready for review</span>}</td><td className="px-4 py-3">{registeredRows.includes(item.row) ? <span className="inline-flex items-center gap-1 text-emerald-700"><Check className="h-4 w-4" /> Registered</span> : item.match_status === 'EXACT MAC MATCH' && item.service_plan && item.record['Installation Date'] ? <button disabled={registeringRow === item.row} onClick={() => void registerMatch(item)} className="rounded-md bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">{registeringRow === item.row ? 'Registering…' : 'Register client'}</button> : <span className="text-xs text-muted-foreground">Excluded</span>}</td></tr>)}
              </tbody>
            </table>
          </div>
        </section>}
