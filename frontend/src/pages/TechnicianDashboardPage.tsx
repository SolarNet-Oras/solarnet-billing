...
const emptyRegistrationForm: RegistrationForm = {
  full_name: '', address: '', contact_number: '', email: '', installation_date: '', service_plan_id: '', binding_lookup: 'mac', mac_address: '', ip_address: '', gps_coordinates: null, location_accuracy_meters: null,
};
...
export default function TechnicianDashboardPage() {
  ...
  const [referralPlanSelections, setReferralPlanSelections] = useState<Record<string, string>>({});
    ...
    {tab === 'tickets' && <section className="space-y-4">
      ...
      {visibleTickets.map((ticket) => {
        ...
        return <article key={ticket.id} className="rounded-2xl border bg-card p-5 shadow-sm">
          ...
          {ticket.referral && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 text-sm text-emerald-950"><p className="font-semibold">Referral installation application</p><p className="mt-1">Contact: {ticket.referral.phone}{ticket.referral.email ? ` · ${ticket.referral.email}` : ''}</p><p className="mt-1">Referred by: {ticket.referral.referrer?.full_name || ticket.customer?.full_name || 'Customer'}{ticket.referral.referrer?.account_number ? ` (${ticket.referral.referrer.account_number})` : ''}</p><p className="mt-2 text-xs text-emerald-800">Confirm the requested service plan before claiming. The prospect receives a separate pending customer record and follows the normal installation approval process.</p></div>}

          {ticket.referral && ticket.ticket_type === 'other' && ticket.workflow_status === 'open' && <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4"><h3 className="font-semibold">Claim this installation application</h3><p className="mt-1 text-xs text-muted-foreground">Select the client’s confirmed plan. Claiming creates a separate pending customer record for the referred person and assigns this installation to you.</p><div className="mt-3 flex flex-col gap-3 sm:flex-row"><select value={referralPlanSelections[ticket.id] || ''} onChange={(event) => setReferralPlanSelections((current) => ({ ...current, [ticket.id]: event.target.value }))} className="min-w-0 flex-1 rounded-lg border bg-background px-3 py-2 text-sm"><option value="">Select confirmed service plan</option>{servicePlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {plan.download_speed}/{plan.upload_speed} Mbps · ₱{Number(plan.price).toLocaleString('en-PH')}/month</option>)}</select><button disabled={busy || !referralPlanSelections[ticket.id]} onClick={() => void runTicketAction(ticket, 'claim-referral-installation', { service_plan_id: referralPlanSelections[ticket.id] }, 'Referral installation claimed. Complete the work and submit the ONU/router MAC for administrator approval.')} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">{busy ? 'Claiming…' : 'CLAIM INSTALLATION'}</button></div></div>}
