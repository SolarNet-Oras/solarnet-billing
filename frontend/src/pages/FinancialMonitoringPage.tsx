...
function FinanceMetricsGraph({ metrics }: { metrics: DailyMetric[] }): React.JSX.Element {
  ...
  const width = 760; const height = 176; const left = 44; const right = 16; const top = 12; const bottom = 28;
  ...
  return <div className="mt-2 overflow-x-auto"><svg viewBox={`0 0 ${width} ${height}`} className="min-w-[30rem] w-full" role="img" aria-label="Daily collections, expenses, and net operating movement graph"><line x1={left} x2={width - right} y1={zeroLine} y2={zeroLine} className="stroke-border" strokeDasharray="3 3" /><text x={4} y={top + 4} className="fill-muted-foreground text-[10px]">{formatPHP(maximum)}</text><text x={4} y={height - bottom + 4} className="fill-muted-foreground text-[10px]">{formatPHP(minimum)}</text><polyline fill="none" stroke="currentColor" className="text-emerald-500" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={points('collections')} /><polyline fill="none" stroke="currentColor" className="text-amber-500" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={points('expenses')} /><polyline fill="none" stroke="currentColor" className="text-sky-500" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={points('net_operating_movement')} />{labels.map((metric) => { const index = metrics.indexOf(metric); return <text key={metric.date} x={x(index)} y={height - 8} textAnchor="middle" className="fill-muted-foreground text-[9px]">{metric.date.slice(8)}</text>; })}</svg></div>;
}
