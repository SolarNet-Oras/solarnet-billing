...
export default function RemittancesPage() {
  ...
  const closePayment = () => { setPaymentInvoice(null); setCheckout(null); setQrCode(''); setSignaturePresent(false); setFamilySigner(false); setFamilySignerName(''); setCashStep('details'); };
  const openPayment = (invoice: Invoice) => { setPaymentInvoice(invoice); setPaymentAmount(String(invoice.balance)); setPaymentMethod('cash'); setReference(''); setCheckout(null); setQrCode(''); setSignaturePresent(false); setFamilySigner(false); setFamilySignerName(''); setCashStep('details'); };
  ...
  const reconcileGcashCheckout = async (quiet = false) => {
    if (!paymentInvoice || !checkout?.checkout_session_id) return;
    try {
      const response = await api.post(`/collector/invoices/${paymentInvoice.id}/gcash-checkouts/${checkout.checkout_session_id}/reconcile`);
