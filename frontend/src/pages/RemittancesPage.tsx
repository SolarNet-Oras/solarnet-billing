...
export default function RemittancesPage() {
  ...
  useEffect(() => { setReturnCounts({}); }, [liquidationTarget?.id]);
  useEffect(() => {
    if (!collector) return;
  ...
  const liquidationReady = cashMatches || (cashShortage && shortageReason === 'travel_expense_gas' && Boolean(expenseReceiptReference.trim()));
  ...
  const liquidate = async () => {
    ...
    setBusy(true);
    try { const form = new FormData(); breakdown.forEach((row, index) => { form.append(`cash_breakdown[${index}][denomination]`, String(row.denomination)); form.append(`cash_breakdown[${index}][kind]`, row.kind); form.append(`cash_breakdown[${index}][count]`, String(row.count)); }); returnBreakdown.forEach((row, index) => { form.append(`cash_return_breakdown[${index}][denomination]`, String(row.denomination)); form.append(`cash_return_breakdown[${index}][kind]`, row.kind); form.append(`cash_return_breakdown[${index}][count]`, String(row.count)); }); if (cashShortage) { form.append('shortage_reason', shortageReason); form.append('expense_receipt_reference', expenseReceiptReference.trim()); if (expenseReceipt) form.append('expense_receipt', expenseReceipt); } const response = await api.post(`/remittances/${liquidationTarget.id}/liquidate`, form); window.alert(response.data?.message || 'Cash liquidated.'); setLiquidationTarget(null); setCounts({}); setReturnCounts({}); setShortageReason(''); setExpenseReceiptReference(''); setExpenseReceipt(null); await load(); }
  ...
  return <DashboardLayout><main className="space-y-6 p-4 md:p-6">
    ...
    {liquidationTarget && cashCounted > cashExpected && <aside className="fixed right-4 top-4 z-[70] max-h-[calc(100vh-2rem)] w-[min(22rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-blue-300 bg-card p-4 shadow-2xl"><h3 className="font-bold text-foreground">Cash return denominations</h3><p className="mt-1 text-xs text-muted-foreground">Cash received {peso(cashCounted)} · remittance {peso(cashExpected)}. Record the bills or coins returned as change.</p><div className="mt-3 grid grid-cols-2 gap-2">{DENOMINATIONS.map(({denomination,kind})=>{const key=denominationKey(denomination,kind);return <label key={key} className="text-xs font-semibold text-foreground">₱{denomination.toLocaleString('en-PH')} {kind}<input min="0" type="number" value={returnCounts[key]||''} onChange={event=>setReturnCounts(current=>({...current,[key]:Math.max(0,Math.trunc(Number(event.target.value)||0))}))} className="mt-1 w-full rounded-lg border bg-background p-2"/></label>})}</div><div className="mt-3 rounded-lg bg-blue-50 p-3 text-sm text-blue-950"><p>Return: <b>{peso(cashReturned)}</b></p><p>Office retains: <b>{peso(cashRetained)}</b></p><p className={cashMatches?'text-emerald-700':'text-amber-700'}>{cashMatches?'Ready: retained cash matches remittance.':`Return ${peso(Math.max(0,cashCounted-cashExpected))} in total.`}</p></div></aside>}
