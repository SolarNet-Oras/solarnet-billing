...
export default function RemittancesPage() {
  ...
  const [excessAction, setExcessAction] = useState<'' | 'return_change' | 'accept_overage'>('');
  ...
  const returnBreakdown = useMemo<CashLine[]>(() => DENOMINATIONS.map(({ denomination, kind }) => ({ denomination, kind, count: Number(returnCounts[denominationKey(denomination, kind)] || 0), amount: denomination * Number(returnCounts[denominationKey(denomination, kind)] || 0) })), [returnCounts]);
  const cashReturned = returnBreakdown.reduce((total, line) => total + line.amount, 0);
  const cashRetained = cashCounted - cashReturned;
  const cashMatches = Math.round(cashExpected * 100) === Math.round(cashRetained * 100);
  const cashShortage = cashRetained < cashExpected;
  const cashExcess = cashCounted > cashExpected;
  const liquidationReady = cashMatches || (cashExcess && excessAction === 'accept_overage' && cashReturned === 0) || (cashShortage && shortageReason === 'travel_expense_gas' && Boolean(expenseReceiptReference.trim()));
  ...
  const liquidate = async () => {
    ...
    setBusy(true);
    try { const form = new FormData(); breakdown.forEach((row, index) => { form.append(`cash_breakdown[${index}][denomination]`, String(row.denomination)); form.append(`cash_breakdown[${index}][kind]`, row.kind); form.append(`cash_breakdown[${index}][count]`, String(row.count)); }); if (cashExcess) form.append('excess_action', excessAction); if (excessAction === 'return_change') returnBreakdown.forEach((row, index) => { form.append(`cash_return_breakdown[${index}][denomination]`, String(row.denomination)); form.append(`cash_return_breakdown[${index}][kind]`, row.kind); form.append(`cash_return_breakdown[${index}][count]`, String(row.count)); }); if (cashShortage) { form.append('shortage_reason', shortageReason); form.append('expense_receipt_reference', expenseReceiptReference.trim()); if (expenseReceipt) form.append('expense_receipt', expenseReceipt); } const response = await api.post(`/remittances/${liquidationTarget.id}/liquidate`, form); window.alert(response.data?.message || 'Cash liquidated.'); setLiquidationTarget(null); setCounts({}); setReturnCounts({}); setExcessAction(''); setShortageReason(''); setExpenseReceiptReference(''); setExpenseReceipt(null); await load(); }
  ...
  const cancelWrongSubmission = async (remittance: Remittance) => {
    if (!window.confirm('Remove this fully refunded wrong submission from the liquidation queue? Payment and refund audit records will remain available.')) return;
    setBusy(true);
    try {
      const response = await api.delete(`/remittances/${remittance.id}`);
      window.alert(response.data?.message || 'Wrong submission removed from the liquidation queue.');
      await load();
    } catch (error: any) {
      window.alert(error.response?.data?.message || 'Could not remove this remittance submission.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return <DashboardLayout><main className="space-y-6 p-4 md:p-6">
    ...
    {collector ? <>
      ...
      const matches = liquidated && Math.round((Number(item.cash_counted_amount || 0) - Number(item.cash_returned_amount || 0)) * 100) === Math.round(Number(totals.cash || 0) * 100);
      const fullyRefunded = Boolean(item.payments?.length) && (item.payments || []).every((payment) => Math.round((payment.refunds || []).reduce((sum, refund) => sum + Number(refund.amount), 0) * 100) >= Math.round(Number(payment.amount) * 100));
      return <article key={item.id} className="rounded-2xl border bg-card p-5">
        ...
        {item.cash_breakdown && <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">{item.cash_breakdown.filter((line) => line.count > 0).map((line) => <span key={`${line.denomination}-${line.kind}`} className="rounded bg-muted px-2 py-1">Received: {line.count} × ₱{line.denomination} = {peso(line.amount)}</span>)}{item.cash_return_breakdown?.filter(line=>line.count>0).map(line=><span key={`return-${line.denomination}-${line.kind}`} className="rounded bg-blue-50 px-2 py-1 text-blue-800">Returned: {line.count} × ₱{line.denomination} = {peso(line.amount)}</span>)}</div>}
        <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => setDetailsTarget(item)} className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-2 text-sm font-semibold text-primary"><Eye className="mr-2 inline h-4 w-4" />View details ({item.payments?.length || 0})</button>{superAdmin && fullyRefunded && item.status === 'submitted' && !liquidated && <button disabled={busy} onClick={() => void cancelWrongSubmission(item)} className="rounded-lg border border-red-300 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 disabled:opacity-50">Delete wrong submission</button>}{item.status === 'submitted' && !liquidated && !fullyRefunded && <button onClick={() => { setLiquidationTarget(item); setCounts({}); }} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"><Calculator className="mr-2 inline h-4 w-4" />Liquidate cash</button>}{item.status === 'submitted' && liquidated && <button disabled={!matches} onClick={() => { setVerifyTarget(item); setReceivedAmount(String(item.declared_amount)); }} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50"><CheckCircle2 className="mr-2 inline h-4 w-4" />Validate received remittance</button>}</div>
      </article>;
    ...
    {liquidationTarget && cashExcess && !showExcessDialog && <button type="button" onClick={() => setShowExcessDialog(true)} className="fixed right-6 top-40 z-[60] rounded-lg border border-blue-300 bg-blue-600 px-3 py-2 text-xs font-semibold text-white shadow-lg hover:bg-blue-700 sm:right-[calc(50%_-_20rem)]">Handle cash change</button>}
    {liquidationTarget && cashExcess && showExcessDialog && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4"><aside className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-blue-300 bg-card p-5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-foreground">Cash change options</h3><p className="mt-1 text-xs text-muted-foreground">Cash received {peso(cashCounted)} · remittance {peso(cashExpected)}.</p></div><button type="button" onClick={() => setShowExcessDialog(false)} aria-label="Close cash change options"><X className="h-5 w-5" /></button></div><div className="mt-4 grid gap-2"><button type="button" onClick={()=>setExcessAction('return_change')} className={`rounded-lg border p-3 text-left text-sm font-semibold ${excessAction==='return_change'?'border-blue-600 bg-blue-50 text-blue-900':'border-border'}`}>Return change with denominations</button><button type="button" onClick={()=>{setExcessAction('accept_overage');setReturnCounts({})}} className={`rounded-lg border p-3 text-left text-sm font-semibold ${excessAction==='accept_overage'?'border-emerald-600 bg-emerald-50 text-emerald-900':'border-border'}`}>Accept excess as audited cash overage</button></div>{excessAction==='return_change'&&<><div className="mt-3 grid grid-cols-2 gap-2">{DENOMINATIONS.map(({denomination,kind})=>{const key=denominationKey(denomination,kind);return <label key={key} className="text-xs font-semibold text-foreground">₱{denomination.toLocaleString('en-PH')} {kind}<input min="0" type="number" value={returnCounts[key]||''} onChange={event=>setReturnCounts(current=>({...current,[key]:Math.max(0,Math.trunc(Number(event.target.value)||0))}))} className="mt-1 w-full rounded-lg border bg-background p-2"/></label>})}</div><div className="mt-3 rounded-lg bg-blue-50 p-3 text-sm text-blue-950"><p>Return: <b>{peso(cashReturned)}</b></p><p>Office retains: <b>{peso(cashRetained)}</b></p><p className={cashMatches?'text-emerald-700':'text-amber-700'}>{cashMatches?'Ready: retained cash matches remittance.':`Return ${peso(Math.max(0,cashCounted-cashExpected))} in total.`}</p></div></>}{excessAction==='accept_overage'&&<p className="mt-3 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-900">The {peso(cashCounted-cashExpected)} excess will be added to Cash as a separate Remittance Cash Overage attributed to the liquidating staff member.</p>}<button type="button" onClick={() => setShowExcessDialog(false)} disabled={!excessAction || (excessAction === 'return_change' && !cashMatches)} className="mt-4 w-full rounded-xl bg-primary px-4 py-2.5 font-semibold text-primary-foreground disabled:opacity-50">Use this cash handling</button></aside></div>}
...
function RemittanceDetailsModal({ remittance, onClose }: { remittance: Remittance; onClose: () => void }) {
  ...
  const refunded = payments.reduce((sum, payment) => sum + (payment.refunds || []).reduce((refundSum, refund) => refundSum + Number(refund.amount), 0), 0);
  const netRetained = total - refunded;
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 sm:p-4" role="dialog" aria-modal="true" aria-label="Remittance collection details">
    <div className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-2xl border bg-card shadow-2xl">
      ...
      <div className="max-h-[calc(92vh-88px)] overflow-auto p-4 sm:p-5">
        ...
        </tbody><tfoot><tr className="border-t bg-muted/40 font-bold"><td colSpan={5} className="p-3 text-right">Gross originally collected</td><td className="p-3 text-right">{peso(total)}</td></tr>{refunded>0&&<><tr className="border-t bg-red-50 font-bold text-red-700 dark:bg-red-950/30 dark:text-red-300"><td colSpan={5} className="p-3 text-right">Cash refunded</td><td className="p-3 text-right">−{peso(refunded)}</td></tr><tr className="border-t bg-emerald-50 font-bold text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300"><td colSpan={5} className="p-3 text-right">Net cash retained</td><td className="p-3 text-right">{peso(netRetained)}</td></tr></>}</tfoot></table></div>
      </div>
    </div>
  </div>;
}
