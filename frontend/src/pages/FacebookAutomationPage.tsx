...
export default function FacebookAutomationPage(): React.JSX.Element {
  ...
  return <DashboardLayout>
    <main className="mx-auto max-w-7xl space-y-5">
      ...
      {isAdmin && status && <section className={`rounded-2xl border p-4 ${status.auto_reply_ready ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900/60 dark:bg-emerald-950/30' : 'border-amber-200 bg-amber-50 dark:border-amber-900/60 dark:bg-amber-950/30'}`}><div className="flex gap-2"><ShieldCheck className={`mt-0.5 h-5 w-5 ${status.auto_reply_ready ? 'text-emerald-600' : 'text-amber-600'}`} /><div><h2 className="font-semibold text-foreground">Automatic Messenger reply: {status.auto_reply_ready ? 'Ready' : 'Needs attention'}</h2>{status.auto_reply_blockers.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-muted-foreground">{status.auto_reply_blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul>}{connected[0]?.webhook_subscribed_at && !connected[0]?.last_webhook_at && <p className="mt-2 text-xs font-medium text-amber-700 dark:text-amber-300">Page subscription was accepted {displayDate(connected[0].webhook_subscribed_at)}, but no signed Messenger event has reached SolarNet yet. Test from a different Facebook account, not the Page account.</p>}{status.last_auto_reply && <p className="mt-2 text-xs text-muted-foreground">Last attempt: {displayDate(status.last_auto_reply.created_at)} · {status.last_auto_reply.delivery_status}{status.last_auto_reply.delivery_error ? ` · ${status.last_auto_reply.delivery_error}` : ''}</p>}<p className="mt-2 text-xs text-muted-foreground">Every new signed Messenger event is processed after the webhook response. The worker should remain running for other background jobs. If AI is temporarily unavailable, SolarNet sends a safe empathetic acknowledgement. Viewing a chat does not stop automation; only explicit Human handoff does.</p>{connected[0] && !connected[0].webhook_subscribed_at && <button type="button" onClick={() => void subscribeWebhook(connected[0])} disabled={busy !== ''} className="mt-3 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-60">{busy === `subscribe:${connected[0].id}` ? 'Subscribing…' : 'Subscribe Page webhook'}</button>}</div></div></section>}
